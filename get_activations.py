...
def get_activations(vdir, sdir, model):
    save_dir = sdir
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
...
def keras_get_activations(video_file_name, sdir, model):

    activations_dir = os.path.join(sdir)
    if not os.path.exists(activations_dir):
        os.makedirs(activations_dir)
    ...
    for idx, layer_name in enumerate(layer_names):
        activations.append(np.array(model.get_layer(layer_name).output))
    for layer in range(len(activations)):
        save_path = os.path.join(activations_dir, video_file_name + "_" + "layer" + "_" + str(layer + 1) + ".npy")
        np.save(save_path, avg_layer_activation)
...
def main():
    ...
    parser.add_argument('-m', '--model_loc', help='location of model', default='./alexnet', type=str)
    args = vars(parser.parse_args())
    get_activations(args['video_data_dir'], args['save_dir'], args['model_loc'])
